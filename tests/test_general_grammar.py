...
def test_midas_main_paths_exclude_linkage_branches() -> None:
    reference_graphs = ROOT / "artifacts/hand_morphology/reference_graphs.json"
    mechanism_bundles = ROOT / "artifacts/hand_morphology/mechanism_bundles.json"
    if not reference_graphs.is_file() or not mechanism_bundles.is_file():
        pytest.skip("reference preprocessing artifacts have not been generated")
    sources = json.loads(
        reference_graphs.read_text()
    )
    hand = next(record for record in sources["hands"] if record["hand_id"] == "midas_hand")
    bundles = json.loads(
        mechanism_bundles.read_text()
    )["bundles"]
