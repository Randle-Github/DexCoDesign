...
if [[ "${MODE}" == "ppo" ]]; then
  ...
  EXPORTS="ALL,TASK_ID=${TASK_ID},HAND_ID_OVERRIDE=${HAND_ID},DEXCODESIGN_REFERENCE_PATH=${REFERENCE},DEXCODESIGN_OBJECT_USD_PATH=${OBJECT_USD},OUTPUT_ROOT=${OUTPUT},RESULT_SET=${TASK_ID},NUM_ENVS_PER_HAND=${NUM_ENVS},MAX_ITERATIONS=${MAX_ITERATIONS},TOTAL_ITERATIONS_BUDGET=${MAX_ITERATIONS}"
  ...
  EXPORTS="ALL,DEXCODESIGN_OBJECT_USD_PATH=${OBJECT_USD},FIXED_REFERENCE=${REFERENCE},OUTPUT_ROOT=${OUTPUT},POPULATION=${POPULATION},PHYSICS_BATCH_SIZE=${PHYSICS_BATCH_SIZE},GENERATIONS=${GENERATIONS},SHARED_PPO_ITERATIONS=${PPO_ITERATIONS},CONTINUE_AFTER_SUCCESS=1,OPTIMIZER_BACKEND=skrl,MORPHOLOGY_CONTEXT=1,GPU_COUNT=${GPU_COUNT},MORPHOLOGY_REPLICAS=${MORPHOLOGY_REPLICAS},PPO_ROLLOUT_MULTIPLIER=${PPO_ROLLOUT_MULTIPLIER}"
