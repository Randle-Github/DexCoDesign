...
runtime-ready robot asset. A generated hand may only be marked
...
5. import the URDF into the intended downstream simulator and verify collision
   coverage and contact behavior;
6. pass the downstream simulator's deterministic load and articulation checks.

Until that exporter and certification command are present, compiled morphology
artifacts should be treated as design outputs rather than certified runtime
assets.
