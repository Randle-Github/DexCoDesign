...
PPO_ROLLOUT_MULTIPLIER="${PPO_ROLLOUT_MULTIPLIER:-32}"
...
exec /opt/slurm/Ubuntu-20.04/24.11.0/bin/sbatch \
  --gpus="a40:${GPU_COUNT}" \
  --export="ALL,GPU_COUNT=${GPU_COUNT},POPULATION=${POPULATION},PHYSICS_BATCH_SIZE=${POPULATION},MORPHOLOGY_REPLICAS=${MORPHOLOGY_REPLICAS},PPO_ROLLOUT_MULTIPLIER=${PPO_ROLLOUT_MULTIPLIER},SHARED_PPO_ITERATIONS=${SHARED_PPO_ITERATIONS},GENERATIONS=${GENERATIONS},CONTINUE_AFTER_SUCCESS=1,OPTIMIZER_BACKEND=skrl,OUTPUT_ROOT=${OUTPUT_ROOT}" \
