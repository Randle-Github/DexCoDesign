#!/usr/bin/env python3
"""Prepare parametric WUJI hand assets and reference manifests for training.

Used by SAC/PPO co-training, fixed-morphology experiments, and validation or
rendering tools. Packages retargeted motion with prototype metadata and builds
candidate USD assets or runtime overlays plus the PhysX batch manifest.
"""
...
def save_reference(
    ...
    if task_targets is not None:
        with np.load(task_targets, allow_pickle=False) as targets:
            count = len(hand_q)
            for key, shape in (("object_pose_wxyz", (count, 7)),
                               ("fingertip_pose_wxyz", (count, 2, 7))):
                value = targets[key]
                if value.shape != shape or not np.isfinite(value).all():
                    raise ValueError(f"Invalid task target {key}: expected finite {shape}")
                values[key] = value.copy()
            for key in ("source_mano_rollout", "source_mano_rollout_sha256", "frame_ids"):
                values[key] = targets[key].copy()
    output.parent.mkdir(parents=True, exist_ok=True)
    np.savez_compressed(output, **values)
...
def main() -> int:
    ...
    parser.add_argument("--task-targets", type=Path, help="Explicit MANO rollout task targets; overrides prototype object and thumb/index goals")
    ...
    if fixed_reference is not None and args.task_targets is not None:
        parser.error("--fixed-reference and --task-targets cannot be combined")
    ...
    for index, candidate_id in enumerate(candidate_ids):
        ...
        if fixed_reference is not None:
            ...
            save_reference(
                ...
                task_targets=args.task_targets,
            )
    ...
    manifest = {
        ...
        "task_targets": None if args.task_targets is None else str(args.task_targets.resolve()),
        "retarget_performed": fixed_reference is None,
        "proxy_used": False,
    }
    path = root / "physx_batch_manifest.json"
    path.write_text(json.dumps(manifest, indent=2) + "\n", encoding="utf-8")
    print(f"WUJI_PARAMETRIC_TRAINING_ASSETS_PREPARED count={count} manifest={path}")
    return 0
