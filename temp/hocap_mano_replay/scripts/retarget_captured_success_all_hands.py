...
def main() -> None:
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "Optional frame cap for smoke tests. By default, retarget the "
            "complete trajectory stored in --capture."
        ),
    )
    ...
    if args.max_frames is not None and args.max_frames < 1:
        parser.error(f"--max-frames must be positive, got {args.max_frames}")
    ...
    captured_frame_count = len(captured["hand_q"])
    frame_count = (
        captured_frame_count
        if args.max_frames is None
        else min(args.max_frames, captured_frame_count)
    )
